...
// Utility: Shuffle choices and update answerIndex
...
function Level1() {
  ...
  const STORAGE_KEY = `level1_state_v1_${studentID}`;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      document.body.style.userSelect = prevUserSelect || "auto";
    };
  }, []);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        let qs = await getQuestions(1);
        if (!qs?.length) {
          setError("No questions available for Level 1.");
          return;
        }
        qs = shuffleArray(qs).slice(0, 8).map((q) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await submitScore(studentID, 1, score, timeTaken, questions.length);
      navigate("/score", {
        state: { studentID, name: studentName, college: collegeName, level: 1, score, total: questions.length, timeTaken },
      });
    } catch {
  ...
  if (!ready)
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-white bg-gradient-to-tr from-indigo-900 via-purple-800 to-pink-700">
        <h1 className="text-3xl font-bold mb-6">Ready to start Level 1?</h1>
        <button
    ...
    <div className="min-h-screen bg-gradient-to-tr from-indigo-900 via-purple-800 to-pink-700 flex items-center justify-center p-6 relative select-none">
      ...
      <div className="max-w-5xl w-full bg-white/20 backdrop-blur-xl rounded-3xl p-10 flex flex-col gap-8">
        ...
        <div className="bg-white/90 p-8 rounded-3xl flex flex-col gap-6">
          <h2 className="text-2xl font-extrabold text-purple-700 animate-gradientText">{q.text}</h2>
